...
def excluir_registro(var_objPlanilha, var_strAba, var_intIdRegistro):
    ...
    var_booPrimeiroItem = True

    for i, linha in enumerate(var_listaAba[1:], start=2):  # pula cabeçalho
        var_strPosicaoRegistro = i 
        if linha[0] == str(var_intIdRegistro):
            print(f"❗ Registro encontrado na linha {i}, acrescentando na lista para exclusão...")
            if not var_booPrimeiroItem:
                var_strPosicaoRegistro -= 1
            var_booPrimeiroItem = False
            var_objAba.delete_rows(var_strPosicaoRegistro)
...
def excluir_atividade(var_objPlanilha, var_intIdRegistro, var_strTipoAtividade):
    ...
    excluir_registro(var_objPlanilha, "atividades", var_intIdRegistro)
    
    print(f"🗑️ Excluindo registro derivado em '{var_strTipoAtividade}'")
    
    excluir_registro(var_objPlanilha, var_strTipoAtividade, var_intIdRegistro)
